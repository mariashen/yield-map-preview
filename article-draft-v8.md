...
**1. Stablecoin demand pulled Treasuries on-chain first, and now Treasury concentration is pulling higher-yield assets after them.**

**2. The dominant strategies that make higher-yield assets feel instant all drag returns toward the risk-free rate.**

**3. Tokenizable yield is abundant. Seven clusters of opportunity explain what can be unlocked on-chain.**

**4. The biggest challenge is distribution.** Of 35 yield-bearing non-stablecoin RWAs above $50M, only two have crossed 2,000 holders. Much of that by design today.

**5. Real-world assets on-chain will grow despite friction.** Two-thirds of stablecoin supply stayed even before the risk-free rate existed on-chain. Now that the base exceeds $300B, structural demand is pulling new RWAs on-chain.
...
That concentration is now pushing stablecoin reserves to diversify and pulling higher-yield assets on-chain. But higher-yield assets are harder to bring on-chain than Treasuries.
...
On-chain capital moves 24/7, settles in seconds, and can be redeployed same-block. Off-chain assets cannot. Every tokenized real world asset has this timing mismatch. The gap has two sides:

**Deployment lag.** Capital deposited on-chain does not earn yield until it reaches the underlying asset. A private credit loan takes weeks to originate. A real estate deal takes months to close. Until the underlying is deployed, capital earns nothing.
...
**B. Spread yield across the pool.** Pool new deposits into an already-earning pool so no individual depositor experiences deployment delay. New capital dilutes the blended rate, but the dilution is a rounding error in a large enough pool. The cost is that existing depositors subsidize new ones. Example: Vaults on Morpho.
...
- **Borrow against RWA tokens on DeFi lending markets.** If the token is listed as collateral on a lending market, holders can borrow stablecoins against it instead of redeeming, even on weekends or outside the fund's redemption window. The fund is never involved. The same mechanic also enables looping, discussed later as a catalyst for new RWA demand.
...
The 34 yield sources with on-chain presence cluster in familiar territory: Treasuries, private credit, corporate bonds. The rest have not reached a meaningful scale. Seven distinct barriers stand in the way.
...
New yield types and new strategies only matter if they reach capital. Today, distribution options are extremely limited.
...
- **Vault curators** like Steakhouse and Gauntlet decide which assets their Morpho vaults lend against¹¹, unlocking distribution to thousands of depositors. Losing a curator closes a distribution channel.
...
Protocol treasuries managing billions want different things than retail holders parking savings. Not everyone wants 3% from Treasuries. Some want 8% from private credit. Some want 15% from a leveraged strategy. That demand is funneling into the same handful of products today, so there is a strong pull for more types of yield.

### B. Concentration in correlated underlying assets & competition for users create hunger for diversification.
...
This concentration is a catalyst. Sky deploys billions across a handful of Treasury and CLO products through Spark and Grove. Every additional dollar it deploys into the same small number of products increases its correlation risk. Sky, Ethena, and Ondo all need uncorrelated, higher-yield sources to diversify their reserves.

The same logic applies to every protocol and DAO treasury sitting in Treasuries today. The concentration in low-yield, correlated assets is the demand engine pulling the next wave of RWAs on-chain. That urgency increases when the underlying assets come under pressure. Private credit default rates have climbed above 5%. Meanwhile, Grove redeemed $327M from JAAA, a tokenized private credit CLO, in a single transaction on March 9. There is an urgent need to diversify risk.

Both risk curators and stablecoins need to attract more users through a differentiated product. If every protocol has access to the same, small number of products, they cannot differentiate. The competition for users drives the need to pull more assets on-chain.
...
A multi-asset vault does not need each position to be individually fast or liquid. Vaults lower the bar for new assets: assets need to be yield-generating and legally wrappable. The curator handles the rest.
...
Opportunities span the full stack:

- New assets to tokenize.
- Infrastructure that makes it easier for new assets to come on-chain (e.g. pooling that aggregates fragmented markets into investable products).
...
- Synthetic products that bring yield exposure on-chain before the underlying asset is fully tokenizable.
...
- New infrastructure that takes each RWA and structures it for more types of users.

Every asset that comes on-chain makes the next one easier to bring, and the infrastructure to support it more valuable.

If you are building in this space, we want to talk.
...
*¹⁵ Morpho Association, ["Cooperation Agreement with Apollo"](https://morpho.org/blog/morpho-association-announces-cooperation-agreement-with-apollo/); [CoinDesk](https://www.coindesk.com/business/2026/02/15/wall-street-giant-apollo-deepens-crypto-push-with-morpho-token-deal), February 2026.*
